"""第 6 课：PPO 强化学习训练脚本 —— 让六足机器人学会走路！

训练流程（每一轮迭代 = 一个"收集经验 -> 学习"的循环）：
    1. 收集经验：1024 个平行世界各跑 24 步（当前策略 + 探索噪声），记录
       每一步的 (观测, 动作, 奖励, 下一个观测)
...
运行方式（在终端里）：
    ...
    cd ~/hexapod-robot/rl
    python train.py --headless                          # 完整训练（默认 1000 轮，约 20 分钟）
    python train.py --headless --max_iterations 50      # 先小跑 50 轮试试水温

参考数据（本机 RTX 5060 实测，1024 个平行世界）：
    400 轮 ≈ 8 分钟，机器人学会稳步前进（10 秒走 1 米、零摔倒）
    平均每轮迭代约 1.2 秒
...
parser.add_argument("--num_envs", type=int, default=1024, help="平行世界数量（GPU 显存不够就减半）")
parser.add_argument("--max_iterations", type=int, default=1000, help="训练轮数（400 轮会走、1000 轮走得更好）")
parser.add_argument("--save_interval", type=int, default=100, help="每多少轮保存一次策略")
parser.add_argument("--log_dir", type=str, default=None, help="日志目录（默认 logs/）")
...
def main():
    ...
    print("[训练] 日志目录:", os.path.join(log_dir, agent_cfg.experiment_name))
    print("[训练] 开始训练：", args_cli.max_iterations, "轮 ×", args_cli.num_envs, "个平行世界 ×",
          agent_cfg.num_steps_per_env, "步")
